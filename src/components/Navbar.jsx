    ...
    <nav className={`navbar${scrolled ? ' scrolled' : ''}`}>
      <div className="navbar-inner container">
        ...
        <ul className="navbar-links">
          ...
          {walletAddress && (
            <li>
              <Link to="/dashboard" style={{ color: 'var(--primary)', fontWeight: 700 }}>DASHBOARD</Link>
            </li>
          )}
        </ul>
        ...
        <div className="navbar-actions" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            ...
            {!walletAddress && (
                <Link to="/signin" className="btn-secondary" style={{ padding: '0.5rem 1rem', fontSize: '0.8rem' }}>SIGN IN</Link>
            )}
        </div>
      </div>
    </nav>
